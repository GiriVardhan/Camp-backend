...
(function($) {
	...
	$(function() {
	    ...
	    $('#importEntityForm').ajaxForm({
	        beforeSubmit: function(a,f,o) {
	            o.dataType = 'json';		//$('#uploadResponseType')[0].value;
	            
	            var entId = $('#entityTypeId')[0].value;
	        ...
	        success: function(data) {
	            ...
	            if (typeof data == 'object'){
	            	if(data.importedUid != null){
	            		$('#showMsg').html(data.msg);
	            		$('#showResult').html('ImportId :'+data.importedUid+'</br>'+
	            							  'Total Records :'+data.totalRecords);
	            		checkResult = fetchImportResults(data);
	            	}			
	            	else{
	            		$('#showMsg').html(data.msg);
	            	}	
	           ...
	           // if(checkResult == null)
	           // 	setTimeout('fetchImportResults(data)', 2000);
	        }
	    });
	    ...
	    function fetchImportResults(data){
	    	...
	    	$.ajax({
				 type:'POST',
				 url: '/app/'+data.client+'/entity/fetch/import/result',
			 	 data: ({importUID : data.importedUid}),		 	 
		    	 ...
		    	 complete: function() { 
		    	      if(endTime == null)
		    	    	 setTimeout(fetchImportResults(dataObject), 2000); //recursion magic
		    	    	  ...
		    	    	  $('#showMsg').html('Please Wait...granting Permissions to Roles on Imported Entities....');
		    	    	  $.ajax({
			    	 			 type:'POST',
			    	 			 url: '/app/'+data.client+'/entity/import/permission/add',
			    	 		 	 data: ({entityTypeId : dataObject.entityTypeId}),		 	 
			    	 		 	 	success: function(data) {
		    	    		  		if(data)
		    	    		  			$('#showMsg').html('Your Import is Completed....');
			    	 	    	 }
			    	 	     });
		    	      }	  
